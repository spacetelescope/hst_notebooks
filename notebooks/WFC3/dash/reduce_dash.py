...
class DashData(object):
    def __init__(self, file_name=None, flt_file_name=None):
        ...
        if "_ima.fits" not in file_name:
            ...
            try:
                ...
                if (
                    (self.ima_file[0].header["INSTRUME"].strip() == "WFC3")
                    & (self.ima_file[0].header["DETECTOR"].strip() == "IR")
                ) == False:
                    raise Exception(
                        "This observation was not performed with WFC3/IR, instrument is set to: {}, and detector is set to: {}".format(
                            self.ima_file[0].header["INSTRUME"],
                            self.ima_file[0].header["DETECTOR"],
                        )
                    )

                ###### Test whether it has more than one science extension (FLTs have only 1)
                nsci = [ext.header["EXTNAME "] for ext in self.ima_file[1:]].count(
                    "SCI"
                )
                if nsci == 1:
                    raise Exception(
                        "This file has only one science extension, it cannot be a WFC3/IR ima"
                    )
                ...
                if bu != "ELECTRONS/S":
                    ...
                    raise Exception(
                        'BUNIT in the "SCI" extensions of this file is set to "{}", but should be set to "ELECTRONS/S"\n'
                        'This is a consequence of UNITCORR set to "{}" and FLATCORR set to "{}".\n'
                        'Please rerun calwf3 on this file after setting both UNITCORR and FLATCORR to "PERFORM" in the 0-th extension header'.format(
                            bu, uc, fc
                        )
                    )
            ...
            if ref_catalog is not None:
                ...
                if create_diff_source_lists is True:
                    diffpath = os.path.dirname(
                        os.path.abspath("diff/{}_*_diff.fits".format(self.root))
                    )
                    cat_images = sorted(
                        [
                            os.path.basename(x)
    ...
    def diff_seg_map(
        self, cat_images=None, remove_column_names=True, nsigma=1.0, sig=5.0, npixels=5
    ):
        """
        ...
        for index, diff in enumerate(input_images, start=1):
            ...
            if not os.path.exists("segmentation_maps"):
                os.mkdir("segmentation_maps")
            hdu.writeto(
                ("segmentation_maps/{}_{:02d}_diff_seg.fits").format(self.root, index),
                overwrite=True,
            )
            ...
            ascii.write(
                tbl,
                "segmentation_maps/{}_{:02d}_diff_source_list.dat".format(
                    self.root, index
                ),
                overwrite=True,
            )
            ...
            if remove_column_names is True:
                ...
                ) as f, open("temp_sl.dat", "w") as out:
                    for x in range(n):
        ...
        if cat_images is not None:
            ...
            y = np.array(
                sorted(
                    glob(
                        ("segmentation_maps/{}_*_diff_source_list.dat").format(
                            self.root
                        )
                    )
                )
            ...
            raise Exception(
                "Need to input list of difference files in order to make source list. List should include full path."
            )
    ...
    def fix_cosmic_rays(self, rm_custom=False, flag=None, **lacosmic_param):
        ...
        crmask, clean = lacosmicx.lacosmicx(
            flt_full[1].data,
            gain=gain,
            readnoise=readnoise,
            objlim=objlim,
        ...
        for exp in asn_exposures:
            ...
            new_cr = (
                (crmask == 1)
                & ((flt["DQ"].data & 4096) == 0)
                & ((seg_data == 0) | ((seg_data > 0) & (flt["SCI"].data < 1.0)))
                & (xi > 915)
                & (yi < 295)
            )
    ...
    def make_pointing_asn(self):
        ...
        hdr[
            "COMMENT"
        ] = "This association table is for the read differences for the IMA."
        ...
        hdu_data = fits.BinTableHDU().from_columns(
            [
                fits.Column(name="MEMNAME", format="14A", array=asn_mem_names),
    ...
    def split_ima(self):
        ...
        dq = np.zeros((NSAMP, shape[0], shape[1]), dtype=np.int32)
        time = np.zeros(NSAMP)

        for i in range(NSAMP):
            cube[NSAMP - 1 - i, :, :] = (
                self.ima_file["SCI", i + 1].data
                * self.ima_file["TIME", i + 1].header["PIXVALUE"]
            )
        ...
        for j in range(1, NSAMP - 1):
            ...
            hdu1 = fits.ImageHDU(
                data=science_data[5:-5, 5:-5],
                header=self.ima_file["SCI", NSAMP - j - 1].header,
                name="SCI",
            )
            ...
            hdu2 = fits.ImageHDU(
                data=err[5:-5, 5:-5],
                header=self.ima_file["ERR", NSAMP - j - 1].header,
                name="ERR",
            )
            ...
            hdu3 = fits.ImageHDU(
                data=dq[j + 1][5:-5, 5:-5],
                header=self.ima_file["DQ", NSAMP - j - 1].header,
                name="DQ",
            )
            ...
            hdu4 = fits.ImageHDU(
                data=(np.zeros((1014, 1014), dtype=np.int16) + 1),
                header=self.ima_file["SAMP", NSAMP - j - 1].header,
                name="SAMP",
            )
            hdu5 = fits.ImageHDU(
                np.zeros((1014, 1014)) + dt[j],
                header=self.ima_file["TIME", NSAMP - j - 1].header,
                name="TIME",
            )
    ...
    def subtract_background_reads(self, subtract=True, reset_stars_dq=False):
        ...
        for ii, exp in enumerate(self.diff_files_list):
            ...
            model = diff[1].data * 0.0 + sky_level
            ...
            if not subtract:
                ...
                if "BG_SUB" not in (key for key in diff[1].header.keys()):
                    ...
                    print(
                        "Keyword BG_SUB already set to {}. Skipping background subtraction.".format(
                            diff[1].header["BG_SUB"]
                        )
                    )
...
def main(
    ...
    diffpath = os.path.dirname(
        os.path.abspath("diff/{}_*_diff.fits".format(myDash.root))
    )
    cat_images = sorted(
        [os.path.basename(x) for x in glob("diff/{}_*_diff.fits".format(myDash.root))]
    )
    sc_diff_files = [diffpath + "/" + s for s in cat_images]
